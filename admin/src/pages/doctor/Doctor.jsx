...
function Doctor() {
  ...
  const handleSaveDoctor = async () => {
    try {
      await axios.put(`http://localhost:5001/api/doctor/update/${currentDoctor._id}`, currentDoctor);
      setDoctors(doctors.map((doc) => (doc._id === currentDoctor._id ? currentDoctor : doc)));
  ...
  const handleAssignDoctor = async (formData) => {
    try {
      const response = await axios.post('http://localhost:5001/api/doctor/assign', formData);
        ...
        <tbody>
          {doctors.map((doctor) => (
            <tr key={doctor._id}>
              ...
              <td className="border border-gray-300 px-4 py-2">{doctor.specialization || 'N/A'}</td>
              <td className="border border-gray-300 px-4 py-2">{doctor.phoneNo || doctor.phone}</td>
              <td className="border border-gray-300 px-4 py-2">{doctor.email}</td>
              <td className="border border-gray-300 px-4 py-2">
                <button onClick={() => handleEditClick(doctor)} className="bg-blue-500 text-white px-2 py-1 rounded mr-2">Update</button>
                <button onClick={() => handleDeleteClick(doctor._id)} className="bg-red-500 text-white px-2 py-1 rounded">Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-50 z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-1/3">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Update Doctor</h2>
        <div className="mb-4">
...
const AssignDoctorModal = ({ isOpen, onClose, onAssign }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-50 z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-1/3">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Assign Doctor</h2>
        <div className="mb-4">
          <label className="block mb-1 text-gray-800">Full Name:</label>
          <input
            className="w-full border px-3 py-2 rounded text-gray-900"
